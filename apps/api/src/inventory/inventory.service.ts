...
import { Inventory, MovementType, Prisma, ProductStatus } from '@prisma/client';
...
export class InventoryService {
  ...
  async getBatchMovements(
    ...
    if (!batch || batch.pharmacyId !== pharmacyId)
  ...
  async adjustBatch(
    ...
    return this.prisma.$transaction(async (tx) => {
      const batches = await tx.$queryRaw<Inventory[]>`
        SELECT * FROM "inventory" WHERE id = ${batchId} FOR UPDATE
      `;
      const batch = batches[0];
      if (!batch || batch.pharmacyId !== pharmacyId || batch.deletedAt)
        throw new NotFoundException('Batch not found');
      if (
        (dto.type === MovementType.DAMAGED ||
          dto.type === MovementType.EXPIRED) &&
        dto.quantity > 0
      ) {
        throw new BadRequestException(
          `${dto.type} adjustments must reduce stock`,
        );
      }
  ...
  async updateBatch(
    ...
    return this.prisma.$transaction(async (tx) => {
      const batch = await tx.inventory.findFirst({
        where: { id: batchId, pharmacyId, deletedAt: null },
      });
  ...
  async deleteBatch(pharmacyId: string, batchId: string, userId: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!batch || batch.pharmacyId !== pharmacyId || batch.deletedAt)
        throw new NotFoundException('Batch not found');
      if (batch.reservedStock > 0)
        throw new BadRequestException(
          'Cannot delete batch with reserved stock',
        );
  ...
  async allocateFEFO(
    ...
    if (remaining > 0) {
      throw new BadRequestException(
        'Insufficient available stock across all batches',
      );
    }
  ...
  async getDashboardMetrics(pharmacyId: string) {
    ...
    return {
      ...
      available:
        Number(agg.totalQuantity || 0) - Number(agg.totalReserved || 0),
  ...
  async getLowStockAlerts(pharmacyId: string) {
    ...
    for (const p of products) {
      const totalAvailable = p.inventory.reduce(
        (sum, b) => sum + (b.quantity - b.reservedStock),
        0,
      );
      const minStock = p.inventory.reduce(
        (max, batch) => Math.max(max, batch.minStock),
        0,
      );
    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          if (dto.importId) {
            const importRecord = await tx.productImport.findUnique({
              where: { id: dto.importId },
            });
            if (!importRecord || importRecord.pharmacyId !== pharmacyId) {
              throw new NotFoundException('Inventory import record not found');
            }
          ...
          for (const row of dto.rows) {
            try {
              ...
              if (existingBatch) {
                ...
                if (newQuantity < existingBatch.reservedStock) {
                  results.failedRows++;
                  results.errors.push({
                    rowId: row.rowId,
                    message:
                      'Imported quantity cannot be less than reserved stock',
                  });
                  continue;
                }
                ...
                await tx.inventory.update({
                  ...
                  data: {
                    ...
                    sellingPrice:
                      row.sellingPrice ?? existingBatch.sellingPrice,
                    expiryDate: new Date(row.expiryDate),
                    minStock: row.minStock ?? existingBatch.minStock,
                    location: row.location ?? existingBatch.location,
                    supplierReference:
                      row.supplierReference ?? existingBatch.supplierReference,
                    supplierName:
                      row.supplierName ?? existingBatch.supplierName,
                    deletedAt: null,
                    deletedBy: null,
                  },
                });
